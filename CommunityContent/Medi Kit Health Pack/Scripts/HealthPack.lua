...
local HEART = HEALTH_PACK:FindChildByName("MediKit")
...
local propDisplayText = script:GetCustomProperty("DisplayText")
...
local function healPlayer(trigger, obj)
    ...
    if not debounce and obj ~= nil and obj:IsA("Player") then
        ...
        HEAL_SOUND_1:Play()
        HEAL_SOUND_2:Play()
		 HEART.visibility = Visibility.FORCE_OFF
        ...
        if numUses ~= 0 then
            ...
            --HEART.visibility = true
			  HEART.visibility = Visibility.FORCE_ON
            debounce = false
        else
            Task.Wait(3)
            HEALTH_PACK:Destroy()
...
--local propMyNewString = script:GetCustomProperties("MyNewString")
--Task.Wait(HEALTH_PACK:GetCustomProperties("MyNewString"))


--// Continuously rotates the heart
